...
#ifdef DEBUG
void print_pr_people(int sum_of_weights,
  const vector < pair < unsigned char, unsigned char > > & nr_people) {
...
int generate_nr_people(int n,
  const vector < int > & weights, vector < pair < unsigned char, unsigned char > > & nr_people) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    nr_people[w].first = 1;
    if (!nr_people[w].second) nr_people[w].second = 1;
    sum_of_weights += w;
  }
  #ifdef DEBUG print_pr_people(sum_of_weights, nr_people);
  #endif
...
bool is_valid_team_weight(int nr_one, int nr_other, int one_weight, int other_weight, int nr_weights,
  const vector < int > & weights,
    const vector < pair < unsigned char, unsigned char > > & nr_people) {
  if (!nr_weights) return false;
  int w = one_weight - weights[nr_weights];
  if (!w) {
    if (nr_one == 1) return true;
    else if (nr_other > 1 && other_weight == one_weight) return false;
    ...
    nr_one - 1 <= nr_people[w].second && is_valid_team_weight(nr_one - 1, nr_other, w, other_weight, nr_weights - 1, weights, nr_people))
    return true;
  else {
    if (!(w = other_weight - weights[nr_weights])) return (nr_other == 1) ? true : false;
    else if (w < 0) return false;
    else if (nr_people[w].first && nr_other - 1 >= nr_people[w].first && nr_other - 1 <= nr_people[w].second && is_valid_team_weight(nr_one, nr_other - 1, one_weight, w, nr_weights - 1, weights, nr_people))
      return true;
    else
      return false;
  }
...
pair < int, int > find_minimum_team_weight_pair(int n, int sum_of_weights,
  const vector < int > & weights,
    const vector < pair < unsigned char,
      unsigned char > > & nr_people) {
  int nr_one_max = n / 2, nr_other_max = 0; // max. number of people for a team if (n & 1)
...
int main(int, char ** ) {
    #ifdef ELAPSED_TIME clock_t start = clock();
    #endif
    int cases;
    cin >> cases;
    while (cases--) {
      ...
      if (n < 2)
        ...
        int sum_of_weights = 0;
        for (int i = 1; i <= n; i++) sum_of_weights += weights[i];
      ...
      #ifdef ELAPSED_TIME cerr << "elapsed time = " <<
