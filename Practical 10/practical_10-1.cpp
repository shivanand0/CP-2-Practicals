...
int solve(int x, int y, int time) {
  if (x < 1 || y < 1 || x > W || y > H) return OCCUPIED;
...
int main() {
  ...
  while (true) {
    ...
    for (int i = 0; i <= W; i++)
      for (int j = 0; j <= H; j++)
        for (int k = 0; k <= t; k++) pos[i][j][k] = UNKNOWN;
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int a = left; a <= right; a++)
        for (int b = top; b <= bottom; b++) pos[a][b][ti] = OCCUPIED;
    }

    int ok = 0;
    for (int i = 1; i <= W; i++)
      for (int j = 1; j <= H; j++)
        if (solve(i, j, t) == EMPTY) ok = 1;
    cout << "Robbery #" << c << ":" << endl;
    if (!ok)
      ...
      for (int i = 1; i <= t; i++) {
        ...
        if (x > 0 && y > 0) {
          ok = 1;
          cout << "Time step " << i << ": The robber has been at " << x << "," << y << "." <<
            endl;
        }
      }
      if (ok == 0)
